...
def Transfer_pytorch_Data(adata):
    """
    Convert AnnData to PyG Data with edge_index validation.

    Dynamic error handling:
    - Removes edges with unmapped cells (NaN values)
    ...
    # Dynamic fix: Remove edges with unmapped cells (NaN values)
    valid_mask = G_df['Cell1'].notna() & G_df['Cell2'].notna()
    if not valid_mask.all():
        n_removed = (~valid_mask).sum()
        print(f"[RuntimeValidator] Removed {n_removed} edges with unmapped cells")
        G_df = G_df[valid_mask]
    ...
    # Dynamic fix: Validate edge indices
    num_nodes = adata.n_obs
    edge_index = np.array([edgeList[0], edgeList[1]])

    # Check for out-of-bounds indices
    max_idx = edge_index.max()
    if max_idx >= num_nodes:
        print(f"[RuntimeValidator] Warning: edge_index max ({max_idx}) >= num_nodes ({num_nodes})")
        valid_edges = (edge_index[0] < num_nodes) & (edge_index[1] < num_nodes)
        edge_index = edge_index[:, valid_edges]
        print(f"[RuntimeValidator] Removed {(~valid_edges).sum()} out-of-bounds edges")
...
def Cal_Spatial_Net_3D(adata, rad_cutoff_2D, rad_cutoff_Zaxis,
    ...
    for it in range(num_section - 1):
        ...
        temp_adata.uns['Spatial_Net']['Section_id_1'] = \
            temp_adata.uns['Spatial_Net']['Cell1'].map(spot_section_trans)
        temp_adata.uns['Spatial_Net']['Section_id_2'] = \
            temp_adata.uns['Spatial_Net']['Cell2'].map(spot_section_trans)
...
def Batch_Data(adata, num_batch_x, num_batch_y, spatial_key=['X', 'Y'], plot_Stats=False):
    ...
    batch_x_coor = [np.percentile(Sp_df[:, 0], (1 / num_batch_x) * x * 100)
                    for x in range(num_batch_x + 1)]
    batch_y_coor = [np.percentile(Sp_df[:, 1], (1 / num_batch_y) * x * 100)
                    for x in range(num_batch_y + 1)]
    ...
    for it_x in range(num_batch_x):
        for it_y in range(num_batch_y):
            ...
            temp_adata = temp_adata[
                temp_adata.obs[spatial_key[0]].map(lambda x: min_x <= x <= max_x)
            ]
            temp_adata = temp_adata[
                temp_adata.obs[spatial_key[1]].map(lambda y: min_y <= y <= max_y)
            ]
...
def leiden_clustering(adata, n_clusters, used_obsm='STAGATE', resolution=None):
    if sc is None:
        raise ImportError("scanpy is required for leiden clustering")

    sc.pp.neighbors(adata, use_rep=used_obsm)

    if resolution is None:
        resolution = search_resolution(adata, n_clusters, method='leiden')
...
def search_resolution(adata, n_clusters, method='leiden', start=0.1, end=3.0, step=0.01):
    if sc is None:
        raise ImportError("scanpy is required")

    for res in np.arange(end, start, -step):
        if method == 'leiden':
            sc.tl.leiden(adata, resolution=res)
            n_unique = len(adata.obs['leiden'].unique())
        else:
            sc.tl.louvain(adata, resolution=res)
            n_unique = len(adata.obs['louvain'].unique())

        if n_unique == n_clusters:
            return res

    raise ValueError(f"Could not find resolution for {n_clusters} clusters")
